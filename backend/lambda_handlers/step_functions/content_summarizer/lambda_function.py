...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Processing batch {batch_index}: S3 URI {sanitize_for_logging(batch_s3_uri)} (indices {batch_start}-{batch_end})")
        ...
        from backend.core.summarization.bedrock_summarizer import BedrockSummarizer
        ...
        logger.info(f"Retrieved {len(batch_items) if batch_items else 0} items from batch {batch_index}")
        ...
        for i in range(0, len(batch_items), sub_batch_size):
            ...
            try:
                ...
                logger.error(f"Error summarizing sub-batch: {sanitize_for_logging(e)}")
        ...
        logger.error(f"Error in Content Summarizer Lambda: {sanitize_for_logging(e)}", exc_info=True)
        raise
