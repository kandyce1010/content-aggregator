...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Original content S3 URI: {sanitize_for_logging(original_content_s3_uri)}")
        ...
        logger.info(f"Retrieved {len(content_items) if content_items else 0} original content items")
        ...
        for batch_result in batch_results:
            ...
            if not summarized_s3_uri:
                logger.warning(f"No summarized S3 URI for batch {sanitize_for_logging(batch_result.get('batch_index', 'unknown'))}")
                continue
                
            logger.info(f"Processing batch result: {sanitize_for_logging(summarized_s3_uri)}, indices {batch_start}-{batch_end}, {summary_count} summaries")
        ...
        logger.error(f"Error in Batch Collector Lambda: {sanitize_for_logging(e)}", exc_info=True)
        raise
