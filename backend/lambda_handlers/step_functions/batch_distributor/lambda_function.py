...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"Processing content from S3 URI: {str(content_s3_uri).replace(chr(10), '').replace(chr(13), '') if content_s3_uri else 'None'}")
        ...
        # Create batch configurations with memory optimization
        batches = []
        for i in range(num_batches):
            ...
            # Get the batch of items (slice creates a new list, not a copy of all data)
        ...
        # Clear the full content list to free memory after processing
        del content_items
        ...
        logger.error(f"Error in Batch Distributor Lambda: {str(e).replace(chr(10), ' ').replace(chr(13), ' ')}", exc_info=True)
        raise
