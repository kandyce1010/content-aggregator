...
class BedrockSummarizer:
    ...
    def _should_summarize(self, item: Dict[str, Any]) -> bool:
        """Smart filtering to reduce unnecessary Bedrock calls"""
        # Skip if already has summary
        if item.get('summary') or item.get('ai_summary'):
            return False
        
        # Skip if content is very short (reduced threshold)
        description = item.get('description', '')
        if len(description) < 50:
            return False
            
        # Only skip very low-relevance content (more permissive)
        if item.get('relevance_score', 1.0) < 0.1:
            return False
    ...
    def batch_summarize(self, items: List[Dict[str, Any]], 
        ...
        """
        Batch summarize content items with smart filtering.
        ...
        """
        # Filter items that actually need summarization
        items_to_process = [item for item in items if self._should_summarize(item)]
        
        logger.info(f"Smart filtering: {len(items)} items → {len(items_to_process)} need summarization")
